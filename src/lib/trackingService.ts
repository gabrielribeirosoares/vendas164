...
function clearLegacyMelhorEnvioTokens(): void {
  if (typeof window === "undefined") return;

  for (let index = localStorage.length - 1; index >= 0; index--) {
    const key = localStorage.key(index);
    if (key === "melhor_envio_token" || key?.startsWith("melhor_envio_token_")) {
...
export async function trackOrder(code: string, storeId: string): Promise<TrackingResult | null> {
  ...
  if (!accessToken) throw new Error("Sessão expirada. Entre novamente para atualizar os rastreios.");

  for (let attempt = 1; attempt <= 2; attempt++) {
    try {
      const response = await fetch(
        `/api/tracking?code=${encodeURIComponent(normalizedCode)}&provider=${provider}&storeId=${encodeURIComponent(storeId)}`,
        { headers: { Authorization: `Bearer ${accessToken}` } },
      );

      if (!response.ok) {
        if (response.status === 401) {
          throw new Error("Sessão expirada. Entre novamente para atualizar os rastreios.");
        }
        if (response.status === 403) {
          throw new Error("Você não tem permissão para atualizar os rastreios desta loja.");
        }
      ...
      if (err.message.startsWith("Sessão expirada") || err.message.startsWith("Você não tem permissão")) {
        throw err;
      }
