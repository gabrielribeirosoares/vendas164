      ...
      <SheetContent className="flex w-full flex-col bg-card p-0 sm:max-w-lg" aria-busy={busy}>
                  ...
                  <article key={item.id} className="rounded-xl border bg-background/50 p-3">
                    <div className="flex gap-3">
                      <div className="size-20 shrink-0 overflow-hidden rounded-lg bg-muted">
                        {item.productSnapshot.image_url ? (
                          <img src={item.productSnapshot.image_url} alt="" className="size-full object-contain" />
                        ) : (
                          <ShoppingBag className="size-full p-5 text-muted-foreground" />
                        )}
                      </div>
                      <div className="min-w-0 flex-1">
                        <h4 className="text-sm font-semibold leading-snug">{item.productSnapshot.model}</h4>
                        <p className="mt-1 text-xs text-muted-foreground">
                          {item.productSnapshot.brand} · {item.productSnapshot.scale}
                          {item.productSnapshot.sku && <span className="font-mono ml-1 text-primary/80">· SKU: {item.productSnapshot.sku}</span>}
                        </p>
                        <p className="mt-2 font-semibold">{brl(item.totalPrice)}</p>
                      </div>
                      <Button variant="ghost" size="icon" disabled={busy} aria-label={`Remover ${item.productSnapshot.model}`} onClick={() => cart.removeItem(item.id)}>
                    ...
                    <div className="mt-3 flex items-center justify-between gap-3">
                      <div className="flex items-center rounded-lg border">
                        ...
                        <span className="min-w-7 text-center tabular-nums" aria-live="polite">{item.quantity}</span>
                        <Button
                          variant="ghost"
                          size="icon"
                      ...
                      <div className="text-right text-xs leading-relaxed text-muted-foreground">
                        <p>Sinal: {brl(item.downPaymentToPay)}</p>
                        <p>
                          {item.selectedInstallment > 1
                            ...
                            : `Saldo: ${brl(item.remainingBalance)}`}
                        </p>
                      </div>
                    </div>
                  </article>
        ...
        {cart.items.length > 0 && (
          <div className="space-y-3 border-t bg-background p-5">
            {errorMessage && <p role="alert" className="rounded-lg border border-destructive/30 bg-destructive/10 p-3 text-sm text-destructive">{errorMessage}</p>}
            ...
            <Button className="h-12 w-full text-base" disabled={busy} onClick={handleCheckout}>
              {reserving ? (
                <>
