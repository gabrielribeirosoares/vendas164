...
export function BlingIntegrationDialog({
  ...
  const connection = useQuery({
    queryKey: ["bling-connection", storeId],
    enabled: open,
  ...
  useEffect(() => {
    ...
    try {
      localStorage.removeItem(`bling_tokens_${storeId}`);
    } catch {
      /* storage disabled */
    }
    setBlingProducts([]);
  ...
  async function fetchBlingProducts(nextPage = 1) {
    ...
    try {
      ...
      setBlingProducts((previous) =>
        nextPage === 1
          ? result.products
          : [...new Map([...previous, ...result.products].map((p) => [p.id, p])).values()]
      );
      setPage(nextPage);
      setHasMore(result.hasMore);
      if (nextPage === 1) setSelectedProductIds([]);
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "Não foi possível consultar o Bling.");
    } finally {
        ...
        <div className="space-y-4 overflow-y-auto flex-1 min-h-0 pr-0.5">
          ...
          {connection.isError && (
            <p role="alert" className="text-sm text-destructive">
              ...
              <Button variant="link" onClick={() => connection.refetch()}>
                Tentar novamente
              </Button>
            </p>
          )}
          {connection.data && !connection.data.configured && (
            <p className="text-sm text-muted-foreground">
              A integração está aguardando configuração pelo administrador.
            </p>
          )}
          ...
          {isConnected && (
            <div className="bg-muted/30 p-3 rounded-xl border border-border/40 space-y-3">
              ...
              {hasMore && (
                <Button variant="outline" disabled={loadingProducts} onClick={() => fetchBlingProducts(page + 1)}>
